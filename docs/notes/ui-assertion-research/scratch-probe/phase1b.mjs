...
async function main() {
  ...
  for (const site of prov) {
    ...
    report.push(row);
    fs.writeFileSync(`facts/v2_${site.id}.json`, JSON.stringify(runs[0]));
    console.log(site.id.padEnd(24), 'v2', String(row.identicalPct).padStart(6), '| geom', String(row.geomPct).padStart(6), '| setStable', row.setStable, '| scrollH', row.scrollH.join('/'));
    fs.writeFileSync(`out/phase1b.shard${SHARD}.json`, JSON.stringify({ N, report, driftTally, examples }, null, 2));
  }
  await browser.close();
}
main();
