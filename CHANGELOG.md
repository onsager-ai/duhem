...
- [breaking] Naming pass: step `within:`→`timeout:`, step `secret:`→`secret_outputs:`, lifecycle `environment:`→`provision:`, `environments:`→`profiles:` (`defaults.profile`, `--profile`), and `inherits: [n]` folded into `inputs: { n: { inherit: true } }`. (#368)
