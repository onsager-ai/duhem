//! File-budget lint — bounds per-file token cost so the inner-loop tax
//! of "open one file" stays small.
//!
//! Counts tokens with `tiktoken-rs` against the `o200k_base` encoding.
//! The BPE merges file is vendored in-tree at
//! `xtask/assets/o200k_base.tiktoken` so CI runs are offline and
//! deterministic — the bytes are pinned in the repo, not implicit in
//! whatever `tiktoken-rs` happens to ship. Counts are within ~10% of
//! Claude's own tokenizer; absolute numbers are stable across machines.
//!
//! ## What's counted
//!
...
//! are skipped. Build outputs (`target`, `node_modules`, `dist`,
//! `build`, `.next`, `.turbo`, `out`, `coverage`) are not walked.
//!
//! ## Allow-list
//!
//! `// budget-allow: <non-empty reason>` anywhere in the file exempts
//! it from the budget. Reason text is mandatory and grep-able.
//!
//! ## Modes
//!
...
/// BPE merges file for the o200k_base encoding. Pinned in-tree so CI
/// is offline and deterministic regardless of which `tiktoken-rs`
/// ships.
const VENDORED_VOCAB: &str = include_str!("../assets/o200k_base.tiktoken");

/// Initial budget — calibrated to keep production source files small
/// enough to read in a single pass. Ratchet downward as the codebase
/// matures.
...
/// stdout. Applies the same prod-content rules as the lint (strip
/// `#[cfg(test)]` for Rust).
...
fn default_scan_roots(root: &Path) -> Vec<PathBuf> {
    vec![root.join("crates"), root.join("xtask").join("src")]
}
...
/// remaining `.rs` files are counted.
fn is_prod_source(rel: &Path) -> bool {
    ...
    for component in s.split('/') {
        if matches!(component, "tests" | "examples" | "benches") {
            return false;
        }
    }
...
"#;
    ...
    fn prod_classification() {
        assert!(is_prod_source(Path::new("crates/duhem-runtime/src/lib.rs")));
        assert!(!is_prod_source(Path::new(
            "crates/duhem-schema/tests/integration.rs"
        )));
        assert!(!is_prod_source(Path::new(
            "crates/duhem-judge/src/aggregate/tests.rs"
        )));
        assert!(!is_prod_source(Path::new(
            "crates/duhem-cli/src/x_tests.rs"
        )));
    }
