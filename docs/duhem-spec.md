...
The existing final screenshot, DOM snapshot, and HAR remain available for compatibility and failure diagnosis (`capture/screenshot` / `capture/dom` / `capture/network`). HAR timing is real when session timing was recorded; unknown byte counts remain honest sentinels. Secrets are redacted before storage: sensitive request/response headers (`authorization`, `cookie`, `set-cookie`, …) always, and a request body is redacted when its request carried a sensitive header (the auth-flow heuristic). Response bodies are captured verbatim — they are the repair signal, and like the DOM snapshot they reflect whatever the page received, so a response that echoes a secret carries it into the evidence. A failing `ui/assert-element` also records where it looked as `capture/target-rect`; optional video recording is enabled with `--capture-video` and retained under the same policy.
