...
Retained browser checks also carry a versioned `capture/session-evidence` document. Its single monotonic millisecond clock aligns step boundaries, `capture/step-screenshot` frames, optional `capture/video`, real request/response/body network timing, and recorded browser performance facts. Performance facts include supported navigation/resource timing, web-vital observations, and long tasks; an unsupported or unobserved metric remains unknown. Replay applies no score and no threshold. The dashboard's Replay view seeks and filters this recorded evidence for investigation only. Runs predating the document continue to render their timeline and ordinary artifacts without reinterpretation.

The existing final screenshot, DOM snapshot, and HAR remain available for compatibility and failure diagnosis (`capture/screenshot` / `capture/dom` / `capture/network`). HAR timing is real when session timing was recorded; unknown byte counts remain honest sentinels. Sensitive request/response headers (`authorization`, `cookie`, `set-cookie`, …) are redacted, and a request body is redacted when its request carried a sensitive header (the auth-flow heuristic). Response bodies and DOM snapshots are captured verbatim except for registered secret values, which are masked at the evidence boundary wherever they appear. Masking is exact-substring, so the repair signal survives — a body is never dropped or truncated, only the credential within it is replaced. A failing `ui/assert-element` also records where it looked as `capture/target-rect`; optional video recording is enabled with `--capture-video` and retained under the same policy.

The `capture/` output-name prefix is reserved for runner-emitted evidence — an authored output under it is rejected at validate time and no action produces it. Captures are never bound as `$steps.*.outputs.*`; screenshots, video, network timing, and performance observations exist for humans and agents to investigate, never as judge input.
...
- `ui/wait` — wait for a fixed `duration` (`TimeoutSpec`, e.g. `500ms` or `2s`) without requiring a browser page. Prefer `ui/assert-element` with `timeout:`; fixed waits are an uncapped debugging and transitional-authoring escape hatch.
...
- First Onsager feature verified using Duhem (manually-authored checks, no AI generation yet). ✅ shipped (#46, #47; refreshed to the spec-plan flow via #79; co-located at `onsager-ai/onsager/.duhem/create-spec-plan/` since #225 P4; environment `up:` / `down:` hooks via #61).
...
|Weeks 9–12 |First Onsager check shipped (manual authoring) ✅ shipped (#46, #47; refreshed to the spec-plan flow #79) — `onsager-ai/onsager/.duhem/create-spec-plan/` (moved #225 P4) |Onsager begins using Duhem on selected PRs                           |
...
> **Reframe (epic #225 — Duhem is a tool).** The dogfood relationship described in this appendix predates the co-location decision (§11.2 Alignment note). Product Verification Definitions now live **with the product** in a co-located `.duhem/` suite (§10.1 Pattern D), not in `onsager-ai/duhem`, and the dogfood is **drift monitoring** — running each product's real VDs with the current Duhem to catch a Duhem regression that would break a consumer. Chreode is the shipped example (`onsager-ai/chreode/.duhem/`): self-gated in its own CI (Mode A) and drift-monitored here (Mode B, `.github/workflows/drift-chreode.yml`). Onsager is the same pattern; its VD relocated to `onsager-ai/onsager/.duhem/create-spec-plan/` in epic #225 P4 (onsager#664), self-gated there behind a dormant lane while Onsager is paused. Everything below about *why* Onsager is a valuable dogfood still holds; only *where the checks live* and *why a `pass` is trustworthy* (mechanical judgment + a self-verified Duhem contract, not hoarded checks) changed.
