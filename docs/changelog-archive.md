...
This is the pre-consolidation long-form changelog for Duhem releases
v0.1.0 through v0.1.8. It was frozen on 2026-07-29 when the history was
split out under spec #360.

This file is frozen: never append to it, edit it, or lint it. The live
ledger is [`CHANGELOG.md`](../CHANGELOG.md), and all new entries belong
there.
...
- [additive] In-process live-progress seam: `EvidenceWriter::with_tee` mirrors every persisted event to an `mpsc` sender post-commit, and `Engine::with_progress` subscribes one — the seam behind `duhem run`’s new live terminal narration (auto on a TTY; `--live` / `--no-live` force it; stdout untouched). No new event kinds; evidence wire format and `schema/duhem.schema.json` unchanged. (#299)
...
- [additive] Self-describing assertions (#302): `assertion_evaluated` now also carries `expr` — the human-readable rule that was checked (the authored `assertions:` line, or `step \`<id>\` satisfied == true` for an implicit judgment), rendered via `Assertion::display`. Additive and backward compatible: `None` on an event predating the field; the dashboard falls back to the raw `detail` operands when absent. `schema/duhem.schema.json` unchanged (evidence-wire only). (#302)
- [additive] Self-describing runs (#302): `run_started` now carries an optional `definition` — a snapshot of the Verification Definition source (raw YAML) the run was judged against, recorded via the new `duhem_evidence::run_started_with_definition` / `RunEngine::with_definition_source`. Makes a run's criteria/check descriptions and assertion rules readable without the source file present. `duhem-dashboard`'s `RunDetail` gains `has_definition: bool`, and a new `GET /api/runs/:id/definition` (alias `/definition.yml`) returns the recorded YAML verbatim (`404` when absent or predating the field). No redaction is applied to the snapshot — the schema has no declared-secret-input mechanism today to redact against; revisit if one is added. Additive and backward compatible; `schema/duhem.schema.json` unchanged (evidence-wire + read-API only). (#302)
...
- [clarifying] Terse authoring is the default (#267 parts 2–3): `duhem validate` now warns — never fails — on the ceremony inference (#267) and implicit judgment (#253) made unnecessary: an identity `outputs: { foo: foo }` binding, and a `satisfied: satisfied` binding paired with a `… == true` assertion. `duhem init`, the getting-started guide, and the `api/call` reference example teach the terse form (observe steps drop `outputs:`; assert steps carry the verdict), keeping a bound `satisfied` or an `outputs:` rename as the escape hatch. Advisory only; no schema-shape or accept/reject change. (#267)
