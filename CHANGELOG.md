...
- [additive] `$runtime.contains` now works on strings, and a type-mismatched assertion gates as `fail` (#259): `contains(haystack, value)` is polymorphic over the haystack shape — a **string** haystack is a literal substring test (`$runtime.contains($steps.home.outputs.body_text, "Example Domain")`, the natural way to assert response *content*, complementing the regex-based `$runtime.matches`), while an **array** haystack keeps its element-membership behavior byte-identically. Separately, an assertion `TypeMismatch` (e.g. `contains(str, int)`, `len(int)`) is now classified as `fail` — an authoring defect that blocks the gate and names `type_mismatch(str, int)` in the evidence detail — instead of the retry-eligible, misleading `inconclusive:environment_error` it produced before; genuine environment/hook failures are unaffected. No schema-shape change (`schema/duhem.schema.json` byte-identical); the documented getting-started §5 substring pattern now runs green. Found via an onboarding cold-start audit. (#259)
- [clarifying] Cleaner cold-start DX for `duhem init` + browser errors (#260): a scaffold created outside a Duhem checkout (the `npm i -g duhem` case) no longer emits a repo-internal `../../schema/duhem.schema.json` path in its `# yaml-language-server: $schema=` header — it falls back to the published raw URL, which editors fetch, so the header resolves for an author who never cloned the repo. And a Playwright sidecar that exits before responding (its dependencies or the Chromium browser not installed) now surfaces an actionable `Run \`duhem browser install\` once and retry.` hint — at both the launch handshake and mid-run — instead of a bare `sidecar closed the connection`. No schema impact. Found via an onboarding cold-start audit. (#260)
