...
> **Reframe (epic #225 — Duhem is a tool).** The dogfood relationship described in this appendix predates the co-location decision (§11.2 Alignment note). Product Verification Definitions now live **with the product** in a co-located `.duhem/` suite (§10.1 Pattern D), not in `onsager-ai/duhem`, and the dogfood is **drift monitoring** — running each product's real VDs with the current Duhem to catch a Duhem regression that would break a consumer. Chreode is the shipped example (`onsager-ai/chreode/.duhem/`): self-gated in its own CI (Mode A) and drift-monitored here (Mode B, `.github/workflows/drift-chreode.yml`). Onsager is the same pattern; its VD (`verifications/onsager-dashboard-create-spec-plan/`, still run through the `duhem/run` action) relocates to `onsager-ai/onsager/.duhem/` in epic #225 P4 (deferred while Onsager is paused). Everything below about *why* Onsager is a valuable dogfood still holds; only *where the checks live* and *why a `pass` is trustworthy* (mechanical judgment + a self-verified Duhem contract, not hoarded checks) changed.
