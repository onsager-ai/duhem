...
Screenshot and DOM-snapshot capture ships for `ui/*` checks (spec #202): by default (`duhem run --capture on-failure`) a ui check that ends with any non-pass assertion records a full-page screenshot and the serialized DOM as `capture/screenshot` / `capture/dom` blob observations on its trace; `always` extends capture to passing checks, `off` disables it. The `capture/` output-name prefix is reserved for runner-emitted evidence — captures exist for humans and agents to look at, and are never judge input. HAR-style network evidence and video recordings are not yet captured.
