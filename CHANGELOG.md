...
- [additive] Checks accept `session:` to seed their browser context with storage state captured by the new `ui/capture-session` action, so a login runs once per run instead of once per check. (#347)
