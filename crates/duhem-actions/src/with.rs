//! Shared `with:` helpers — currently just `TimeoutSpec`, the duration
//! shape every UI action accepts under `timeout:`.
//!
...
pub struct TimeoutSpec(pub Duration);

impl From<TimeoutSpec> for Duration {
    fn from(w: TimeoutSpec) -> Self {
        w.0
    }
}
...
impl<'de> Deserialize<'de> for TimeoutSpec {
    fn deserialize<D: Deserializer<'de>>(d: D) -> Result<Self, D::Error> {
        ...
        match Raw::deserialize(d)? {
            Raw::Int(ms) => Ok(TimeoutSpec(Duration::from_millis(ms))),
            Raw::Str(s) => parse_duration_str(&s).map(TimeoutSpec).ok_or_else(|| {
...
mod tests {
    ...
    #[test]
    fn parses_int_ms() {
        let w: TimeoutSpec = serde_yml::from_str("250").unwrap();
        assert_eq!(Duration::from(w), Duration::from_millis(250));
    }

    #[test]
    fn parses_ms_suffix() {
        let w: TimeoutSpec = serde_yml::from_str("\"200ms\"").unwrap();
        assert_eq!(Duration::from(w), Duration::from_millis(200));
    }

    #[test]
    fn parses_seconds() {
        let w: TimeoutSpec = serde_yml::from_str("2s").unwrap();
        assert_eq!(Duration::from(w), Duration::from_secs(2));
    }

    #[test]
    fn parses_minutes() {
        let w: TimeoutSpec = serde_yml::from_str("1m").unwrap();
        assert_eq!(Duration::from(w), Duration::from_secs(60));
    }

    #[test]
    fn rejects_garbage() {
        assert!(serde_yml::from_str::<TimeoutSpec>("\"forever\"").is_err());
    }
    ...
    fn rejects_minutes_overflow() {
        ...
        assert!(serde_yml::from_str::<TimeoutSpec>(&huge).is_err());
    }
}
