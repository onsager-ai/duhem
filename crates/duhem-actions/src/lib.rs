...
pub use browser::{
    BrowserPerformanceObservation, BrowserSessionEvidence, CheckBrowser, Cookie, ElementState,
    NetworkBatch, NetworkEvent, Page, PwError, Rect, RunBrowser, SelectBy,
};
pub use cli::Invoke;
...
pub use ui::{
    AssertElement, AssertState, AssertUrl, CaptureSession, Click, Extract, Navigate, Select, Type,
    Wait, validate_field_source,
};
...
mod catalog_tests {
    ...
    fn catalog_is_complete_with_unique_uses() {
        let cat = catalog();
        assert_eq!(cat.len(), 18, "expected 18 action contracts");
        let mut uses: Vec<&str> = cat.iter().map(|c| c.uses).collect();
        uses.sort();
        uses.dedup();
        assert_eq!(uses.len(), 18, "every `uses` is unique");
    }
    ...
    fn contract_for_resolves_and_carries_the_documented_output() {
        ...
        assert_eq!(
            contract_for("ui/capture-session").unwrap().secret_outputs,
            vec!["state"]
        );
