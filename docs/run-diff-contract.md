...
- **verdict / state values** — criteria, checks, and evaluated assertions
  use `"pass"`, `"fail"`, `"inconclusive:<cause>"`, or `null` (no
  recorded verdict). An assertion whose operands come from a skipped
  step has no recorded entry.
