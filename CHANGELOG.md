...
- [clarifying] Assertions and verdicts share one three-state vocabulary; `skipped` is an execution fact on steps only, and gating follows judgment rather than a raw `satisfied` observation. (#377)
- [breaking] A failing step now blocks the rest of its check by default; steps accept `if: success|always|failure` to opt out. Add `if: always` to steps that must still run after a failure. (#377)
